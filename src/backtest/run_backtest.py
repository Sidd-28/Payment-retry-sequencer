...
def aggregate(audit_df: pd.DataFrame, gateway_fee: float, annoyance_cost: float) -> dict:
    ...
    false_positive = dict(
        ...
        policy=dict(
            ...
            # cost attribution: gateway_fee is charged per retry regardless
            # of outcome, and annoyance_cost is charged per FAILED retry -
            # both are already summed into gateway_cost_policy /
            # annoyance_cost_policy above (and therefore into
            # net_recovered_policy). This just isolates the slice of that
            # already-deducted cost that came specifically from wasted
            # (gt_is_recoverable == False) retries, so the deduction is
            # visible rather than buried in the aggregate.
            wasted_gateway_cost=float(audit_df.loc[policy_wasted_mask, "policy_gateway_fee"].sum()),
            wasted_annoyance_cost=float(audit_df.loc[policy_wasted_mask, "policy_annoyance_cost"].sum()),
        ),
        baseline_b=dict(
            ...
            wasted_gateway_cost=float(audit_df.loc[gt_unrecoverable_mask, "baseline_b_gateway_fee"].sum()),
            wasted_annoyance_cost=float(audit_df.loc[gt_unrecoverable_mask, "baseline_b_annoyance_cost"].sum()),
        ),
    )
    false_positive["policy"]["wasted_total_cost"] = (
        false_positive["policy"]["wasted_gateway_cost"] + false_positive["policy"]["wasted_annoyance_cost"]
    )
    false_positive["baseline_b"]["wasted_total_cost"] = (
        false_positive["baseline_b"]["wasted_gateway_cost"] + false_positive["baseline_b"]["wasted_annoyance_cost"]
    )
    # Hypothetical: net recovered if the wasted (FP) retries had simply not
    # been attempted at all. Adds back their cost; treats their (near-zero,
    # since gt_is_recoverable == False) revenue contribution as unchanged -
    # a reasonable approximation, not an exact counterfactual, since a small
    # number of gt-unrecoverable rows can still fluke-recover.
    false_positive["policy"]["net_recovered_if_no_fp_retries"] = (
        net_recovered_policy + false_positive["policy"]["wasted_total_cost"]
    )
...
# Embedded fallback used ONLY when no external phase4_report_template.md can
# be found (neither at --template's path nor alongside this script). This is
# the SAME content as the phase4_report_template.md file shipped alongside
# this script - kept in sync manually. Its purpose is to guarantee the
# written report always includes every section this script computes (net
# recovered, false-positive rate, guardrail compliance, latency distribution,
# cost breakdown, sensitivity table) even if the external template file gets
# lost, renamed, or run from a different working directory - instead of
# silently degrading to a single "{{POLICY_NET}}" token, which is what used
# to happen here and is why past reports sometimes contained only the
# headline dollar figure.
DEFAULT_TEMPLATE_TEXT = """# Payment Retry Sequencer — Phase 4 Backtest Report

Run: {{RUN_TIMESTAMP}}
Input: {{INPUT_SOURCE}}
Seed: {{SEED}}
Transactions: {{N_TRANSACTIONS}} (total at-risk amount: {{TOTAL_AT_RISK}})
...
|---|---|---|---|---|---|
| **Policy engine** | {{POLICY_GROSS}} | {{POLICY_NET}} | {{POLICY_NET_PCT}} | {{POLICY_RETRIES}} | {{POLICY_RECOVERED_N}} |
| Baseline A (no retry ever) | {{BASELINE_A_NET}} | {{BASELINE_A_NET}} | {{BASELINE_A_NET_PCT}} | 0 | 0 |
| Baseline B (naive retry-once) | {{BASELINE_B_GROSS}} | {{BASELINE_B_NET}} | {{BASELINE_B_NET_PCT}} | {{BASELINE_B_RETRIES}} | {{BASELINE_B_RECOVERED_N}} |

Policy vs. Baseline A (net): {{POLICY_VS_A_DELTA}}
Policy vs. Baseline B (net): {{POLICY_VS_B_DELTA}}

Policy costs already subtracted into net: gateway {{POLICY_GATEWAY_COST}}, annoyance/friction {{POLICY_ANNOYANCE_COST}}
Baseline B costs already subtracted into net: gateway {{BASELINE_B_GATEWAY_COST}}, annoyance/friction {{BASELINE_B_ANNOYANCE_COST}}
...
Total transactions with `gt_is_recoverable == False`: {{FP_N_UNRECOVERABLE}}

| | Wasted retries (n) | FP rate (of unrecoverable) | Amount at risk in wasted retries | Share of all retries fired |
|---|---|---|---|---|
| **Policy engine** | {{FP_POLICY_N}} | {{FP_POLICY_RATE}} | {{FP_POLICY_AMOUNT}} | {{FP_POLICY_SHARE_OF_RETRIES}} |
| Baseline B (naive) | {{FP_BASELINE_B_N}} | {{FP_BASELINE_B_RATE}} | {{FP_BASELINE_B_AMOUNT}} | {{FP_BASELINE_B_SHARE_OF_RETRIES}} |
...
**Yes — the net recovered figure above already subtracts a per-retry cost for false-positive retries.** Every retry (gateway fee) and every *failed* retry (annoyance/friction cost) is charged into the net figure regardless of whether it was a wasted (false-positive) retry or a legitimate one; the table below isolates the slice of that already-applied deduction attributable specifically to the {{FP_POLICY_N}} wasted retries, so the effect is visible rather than buried in the aggregate.

| | Gateway fee cost | Annoyance/friction cost | Total cost from wasted retries |
|---|---|---|---|
| Policy engine | {{FP_POLICY_WASTED_GATEWAY_COST}} | {{FP_POLICY_WASTED_ANNOYANCE_COST}} | {{FP_POLICY_WASTED_TOTAL_COST}} |

Net recovered if these wasted retries had never been attempted (approximate — assumes their own revenue contribution is unchanged, since `gt_is_recoverable == False` implies it was already near-zero): **{{FP_POLICY_NET_IF_NO_FP}}** vs. the actual net of {{POLICY_NET}}.
...
- **Engine invariant (defines PASS/FAIL — should always be exactly 0):** {{GUARDRAIL_ENGINE_VIOLATIONS}} retries recommended when the engine itself saw a risk_block/card_expired prediction or risk-coded bank response code.
- **For context only — true-category leakage:** {{GUARDRAIL_GT_VIOLATIONS}} retries were recommended on rows whose *true* category was risk_block/card_expired ({{GUARDRAIL_GT_VIOLATIONS_PCT}} of such rows), purely because the classifier misclassified them into a retry-eligible category before the engine ever saw them. This is a model-accuracy gap, not a guardrail bug — the engine can only gate on what it's told.

{{GUARDRAIL_TABLE}}

Rows where `gt_is_recoverable`/`gt_ideal_retry_action` disagreed with a risk_block/card_expired `decline_reason_category` (data-quality flag, not a policy issue): {{GUARDRAIL_LABEL_MISMATCH_N}}
...
### Policy engine
{{POLICY_LATENCY_BUCKET_TABLE}}

### Baseline B (naive retry-once)
{{BASELINE_B_LATENCY_BUCKET_TABLE}}

(Raw stats — policy: {{POLICY_LATENCY}}; baseline B: {{BASELINE_B_LATENCY}})

## Safety / Other Compliance Detail

- Hard-rule violations (Layer-1 invariant): {{HARD_RULE_VIOLATIONS}}
- Residual leakage (retries fired on truth-hard-blocked rows via misclassification): {{RESIDUAL_LEAKAGE_N}} ({{RESIDUAL_LEAKAGE_RATE}})
- Naive baseline hard-FP count/rate (always retries everything): {{NAIVE_HARD_FP_N}} ({{NAIVE_HARD_FP_RATE}})
- Over-block count/amount (hard rule fired on a non-hard-blocked truth category): {{OVER_BLOCK_N}} / {{OVER_BLOCK_AMOUNT}} (expected recovery forgone: {{OVER_BLOCK_FORGONE}})
- "other" category policy gap (schema says no-automation, code doesn't hard-gate it): {{OTHER_GAP_N}}
- Flagged for human review: {{FLAGGED_N}}

{{RISK_CODE_PLACEHOLDER_WARNING}}

## Cost Assumptions & Sensitivity

Gateway fee assumption: {{GATEWAY_FEE_ASSUMPTION}} per retry attempt
Annoyance cost assumption: {{ANNOYANCE_COST_ASSUMPTION}} per failed retry
...
def render_report(template_text: str, results: dict, sensitivity: list, meta: dict) -> str:
    ...
    tokens = {
        ...
        "{{FP_POLICY_WASTED_GATEWAY_COST}}": money(fp['policy']['wasted_gateway_cost']),
        "{{FP_POLICY_WASTED_ANNOYANCE_COST}}": money(fp['policy']['wasted_annoyance_cost']),
        "{{FP_POLICY_WASTED_TOTAL_COST}}": money(fp['policy']['wasted_total_cost']),
        "{{FP_POLICY_NET_IF_NO_FP}}": money(fp['policy']['net_recovered_if_no_fp_retries']),
    ...
    # Safety net: if any {{TOKEN}} placeholders survive, the template
    # referenced something render_report() doesn't produce (e.g. a hand-
    # edited template with a typo'd token name) - that would silently
    # ship a report with literal "{{...}}" text sitting in it instead of
    # data. Surface that loudly instead of letting it pass quietly.
    leftover = sorted(set(re.findall(r"\{\{[A-Z0-9_]+\}\}", text)))
    if leftover:
        print(f"[phase4] WARNING: {len(leftover)} unreplaced template token(s) "
              f"left in the rendered report: {', '.join(leftover)}. Check that "
              f"the template matches the tokens render_report() actually "
...
def main():
    ...
    if not template_path.exists():
        # fall back to a copy alongside this script (handles running from a
        # different working directory than where --template's relative path
        # points)
        alongside = Path(__file__).parent / REPORT_TEMPLATE_PATH_DEFAULT
        template_path = alongside if alongside.exists() else None

    if template_path is not None:
        template_text = template_path.read_text()
        print(f"[phase4] using report template: {template_path}", file=sys.stderr)
    else:
        # No external template file found anywhere - previously this fell
        # back to the single token "{{POLICY_NET}}", which produced a report
        # containing ONLY the headline dollar figure even though every other
        # metric (false-positive rate, guardrail compliance, latency
        # distribution, cost breakdown) had already been computed. Fall back
        # to the full embedded template instead so the written report is
        # always complete regardless of where the script is run from.
        template_text = DEFAULT_TEMPLATE_TEXT
        print(f"[phase4] WARNING: no template file found at {args.template!r} "
              f"or alongside run_backtest.py ({Path(__file__).parent / REPORT_TEMPLATE_PATH_DEFAULT}) "
              f"- using the built-in default template embedded in this script. "
              f"Copy phase4_report_template.md next to run_backtest.py (or pass "
              f"--template <path>) if you want to customize the report layout.",
              file=sys.stderr)
